...
def main():
    ...
    file_object = open('gtsrb_final_log_' + model_name + '_' + str(process_no) + '.txt', 'a')
    inputs = np.load("gtsrb_test_data.npy")
    labels = np.load("gtsrb_test_label.npy")
    total = 10000
    ...
    file_object.flush()
    K.clear_session()
    model = tf.keras.models.load_model('Model/gtsrb_' + model_name + '.h5')

    low = 5000 * process_no
    high = 5000 * (process_no + 1)
    ...
    for i in range(total):
        if low <= i < high:
            predicted_label = model.predict(tf.expand_dims(np.transpose(inputs[i], [1, 2, 0]), axis=0)).argmax(axis=-1)[0]
    ...
    for i in range(total):
        if low <= i < high:
            if count in correct_indices:
                for j in range(5):
                    res = tfi_batch.inject(model=model, x_test=tf.expand_dims(np.transpose(inputs[i], [1, 2, 0]), axis=0),
